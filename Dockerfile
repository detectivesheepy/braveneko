# This Dockerfile is pre-processed by the ./utils/docker script, it is not meant to be used directly.

FROM ghcr.io/m1k1o/neko/base:latest AS server
FROM ghcr.io/m1k1o/neko/base:latest AS client
FROM ghcr.io/m1k1o/neko/base:latest AS xorg-deps
FROM ghcr.io/m1k1o/neko/brave:latest AS runtime

# tells neko-rooms which version of the API to use
LABEL net.m1k1o.neko.api-version=3

COPY --from=server /src/bin/plugins/ /etc/neko/plugins/
# Original: COPY --from=server /src/bin/neko /usr/bin/neko
COPY --from=server /usr/bin/neko /usr/bin/neko
COPY --from=client /src/dist/ /var/www
COPY --from=xorg-deps /usr/local/lib/xorg/modules/drivers/dummy_drv.so /usr/lib/xorg/modules/drivers/dummy_drv.so
COPY --from=xorg-deps /usr/local/lib/xorg/modules/input/neko_drv.so /usr/lib/xorg/modules/input/neko_drv.so

COPY config.yml /etc/neko/neko.yaml
